...
BOT_USERNAME = os.getenv("BOT_USERNAME")  # Add this for referral links
SUPABASE_URL = os.getenv("SUPABASE_URL")
SUPABASE_KEY = os.getenv("SUPABASE_KEY")
ADMIN_CHAT_ID = os.getenv("ADMIN_CHAT_ID")
WEBHOOK_URL = os.getenv("WEBHOOK_URL")  # e.g., https://backend-python-6q8a.onrender.com/hook/central_bot
VERIFY_KEY = os.getenv("VERIFY_KEY")  # Optional for verify_booking
...
def compute_tier(points: int) -> str:
    tier = "Bronze"
    for name, threshold in reversed(TIER_THRESHOLDS):
        if points >= threshold:
            tier = name
            break
    return tier
...
async def get_points_awarded_today(user_id: str) -> int:
    ...
    try:
        ...
        return sum(int(r["points"]) for r in rows if r["points"] > 0)
    except Exception:
...
async def award_points(user_id: str, delta: int, reason: str, booking_id: Optional[str] = None) -> dict:
    ...
    Award delta points to user_id:
      - enforces DAILY_POINTS_CAP (only for positive delta)
      - updates central_bot_leads.points and tier
      - inserts points_history
      - for 'booking_verified:<something>' attempts to award referrer bonus (idempotent)
    ...
    if delta < 0:
        ...
        # Daily cap check for positive awards
        awarded_today = await get_points_awarded_today(user_id)
        if awarded_today + delta > DAILY_POINTS_CAP:
    ...
    # Referral bonus (best-effort, idempotent) for booking_verified reasons
...
async def has_history(user_id: str, reason: str) -> bool:
    """Return True if points_history already has this exact reason for user."""
    def _q():
    ...
    try:
        ...
        return False  # Assume not awarded on error to avoid duplicates
...
async def generate_discount_code(chat_id: int, business_id: str, discount_id: str) -> tuple[str, str]:
    ...
    # Award points for claiming promo (idempotent, unique per discount)
    try:
        user_row = await supabase_find_registered(chat_id)
        if user_row:
            claim_reason = f"claim_promo:{discount_id}"
            if not await has_history(user_row["id"], claim_reason):
                await award_points(user_row["id"], POINTS_CLAIM_PROMO, claim_reason)
    except Exception:
...
async def handle_message_update(message: Dict[str, Any]):
    ...
    if contact and state.get("stage") == "awaiting_phone_profile":
        ...
        try:
            if registered:
                user_row = registered  # Already fresh
                if user_row.get("dob") and user_row.get("phone_number"):
                    user_id = user_row["id"]
                    profile_reason = "profile_complete"
                    if not await has_history(user_id, profile_reason):
                        await award_points(user_id, POINTS_PROFILE_COMPLETE, profile_reason)
        except Exception:
    ...
    if text.lower().startswith("/start"):
        referral_id = None
        if text.lower() != "/start":
            referral_id_str = text[len("/start "):]
            try:
                referral_id = str(uuid.UUID(referral_id_str))
            except ValueError:
                logger.error(f"Invalid referral ID: {referral_id_str}")
        registered = await supabase_find_registered(chat_id)
        if registered:
...
async def handle_callback_query(callback_query: Dict[str, Any]):
    ...
    if callback_data == "menu:main":
        ...
        set_state(chat_id, state)
        return {"ok": True}
    elif callback_data == "menu:refer":
        if registered:
            user_id = registered["id"]
            referral_link = f"t.me/{BOT_USERNAME}?start={user_id}"
            await send_message(chat_id, f"Share this link to refer friends: {referral_link}\nThey'll get starter points, and you'll get {POINTS_REFERRAL_JOIN} points when they join, plus {POINTS_REFERRAL_VERIFIED} when they complete a verified booking!")
        else:
            await send_message(chat_id, "Register first to refer friends.")
        return {"ok": True}
    ...
    if state.get("stage") == "awaiting_interests":
        if callback_data.startswith("interest:"):
            ...
            if entry_id:
                ...
                try:
                    # Award starter points (idempotent)
                    ...
                    # If referred_by, set it and award referrer join points (idempotent per referral)
        ...
        if callback_data == "menu:points":
            ...
            try:
                ...
                for d in discounts:
                    ...
                    await send_message(chat_id, message, keyboard)
                    # Award view discount points (idempotent per discount)
                    view_reason = f"view_discount:{d['id']}"
                    if not await has_history(registered["id"], view_reason):
                        await award_points(registered["id"], POINTS_VIEW_DISCOUNT, view_reason)
            except Exception as e:
            ...
            try:
                ...
                if not business:
                    await send_message(chat_id, "Business not found.")
                    return {"ok": True}
                # Create a booking record (pending) and award booking-created points
                try:
                    booking_payload = {
                        "user_id": registered["id"],
                        "business_id": business_id,
...
async def verify_booking(request: Request):
    ...
    Header (optional):
      x-verify-key: secret if VERIFY_KEY set
    ...
    try:
        ...
        promo_row = resp.data[0] if (hasattr(resp, "data") and resp.data) else None
        table_name = "user_giveaways" if promo_row else None
        ...
        if not promo_row:
            ...
            promo_row = resp2.data[0] if (hasattr(resp2, "data") and resp2.data) else None
            table_name = "user_discounts" if promo_row else None

        if not promo_row:
            return PlainTextResponse("Promo not found", status_code=404)

        telegram_id = promo_row.get("telegram_id")
        if not telegram_id:
        ...
        # Create or update booking to completed
        def _find_booking():
            return supabase.table("user_bookings").select("*").eq("user_id", user_id).eq("business_id", business_id).order("booking_date", desc=True).limit(1).execute()
        ...
        # Award verified booking points (idempotent per promo_code)
        verified_reason = f"booking_verified:{promo_code}"
        if not await has_history(user_id, verified_reason):
            await award_points(user_id, POINTS_BOOKING_VERIFIED, verified_reason, booking_id)

        # Update promo row to redeemed
        try:
            await supabase_update_by_id_return(table_name, promo_row["id"], {"entry_status": "redeemed", "redeemed_at": now_iso()})
        except Exception:
