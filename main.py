...
app = FastAPI(title="Multi-Business Telegram Bot", version="2.0.0")
...
# Run webhook initialization on startup
...
async def admin_notify_city(request: Request, x_admin_secret: str = Header(...)):
    logger.info(f"Admin notify request headers: {dict(request.headers)}")
    ...
    try:
        ...
        # Query users in the specified city
        def _get_users():
            return supabase.table("central_bot_leads").select("telegram_id").eq("is_draft", False).eq("city", city).execute()
        ...
        async with httpx.AsyncClient(timeout=httpx.Timeout(20.0)) as client:
            for user_id in user_ids:
                try:
                    ...
                    logger.error(f"Failed to send notification to user {user_id}: HTTP {e.response.status_code}")
                except Exception as e:
...
@app.get("/")
async def root():
    return {"message": "Multi-Business Telegram Bot is running!", "version": "2.0.0"}
